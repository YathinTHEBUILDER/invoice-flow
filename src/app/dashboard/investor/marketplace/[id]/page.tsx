...
export default async function InvestmentDetailPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    // 1. Fetch funding request details with error resilience
    ...
    // 2. Fetch current investment state
    const totalInvestedResult = await db
      .select({ sum: sum(investments.amount) })
      .from(investments)
      .where(eq(investments.fundingRequestId, id));
    
    const currentSum = parseFloat(totalInvestedResult[0]?.sum || "0");
    const requestedAmt = parseFloat(request.requestedAmount);
    const remaining = Math.max(0, requestedAmt - currentSum);
    const progress = requestedAmt > 0 ? (currentSum / requestedAmt) * 100 : 0;

    // 3. Fetch user profile for wallet balance check
    const userRecord = await db.query.users.findFirst({ 
      where: eq(users.id, user.id) 
    });
    
    const now = new Date();
    const dueDate = request.invoice.dueDate ? new Date(request.invoice.dueDate) : now;
      ...
      <div className="flex-1 space-y-8 p-2">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start gap-8">
          <div className="flex-1 space-y-4">
            ...
            <h1 className="text-4xl font-extrabold tracking-tight">{request.invoice.msme.companyName || "Verified Borrower"}</h1>
                ...
                <div className="p-4 rounded-xl bg-muted/30 border border-muted-foreground/5 space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-bold">
                      {request.invoice.dueDate ? new Date(request.invoice.dueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'TBD'}
                    </p>
            ...
            <Card className="border-none shadow-md overflow-hidden">
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed italic">
                  "{request.invoice.msme.companyName || "This borrower"} has demonstrated consistent operational efficiency and financial discipline. This invoice is part of their recurring B2B cycle with institutional debtors."
                </p>
              </CardContent>
            </Card>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <Card className="max-w-md w-full border-rose-500/20 bg-rose-500/5 shadow-2xl shadow-rose-500/5 overflow-hidden">
          ...
          <CardContent className="pt-10 pb-10 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-rose-950">Asset Unavailable</h3>
              <p className="text-sm text-rose-800/70">
                We encountered an error retrieving the details for this investment opportunity. 
            ...
            <div className="pt-4 flex flex-col gap-2">
              <Button asChild variant="outline" className="border-rose-200 hover:bg-rose-100">
                <Link href={`/dashboard/investor/marketplace/${id}`} className="flex items-center gap-2">
