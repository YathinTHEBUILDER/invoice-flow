...
export function InvestmentForm({ fundingRequestId, remainingLimit }: InvestmentFormProps) {
  ...
  const estimatedReturn = amount ? (parseFloat(amount) * 0.125 * (90/365)).toFixed(2) : "0.00";
    ...
    <form onSubmit={handleInvest} className="space-y-6">
      <div className="space-y-3">
        <div className="relative">
          ...
          <Input
            ...
            disabled={isPending || remainingLimit <= 0}
        ...
        {amount && !isNaN(parseFloat(amount)) && (
          <div className="flex items-center justify-between px-2 py-3 bg-primary/5 rounded-lg border border-primary/10 animate-in fade-in slide-in-from-top-1">
            <div className="space-y-0.5">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Est. Return (90 Days)</p>
      ...
      <div className="space-y-4">
        <Button 
          ...
          disabled={isPending || !amount || parseFloat(amount) < 1000 || remainingLimit <= 0}
        >
          {isPending ? (
            <>
